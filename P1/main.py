...
class md5(object):
    ...
    def __init__(self, string='', state=None, count=0):
        ...
        if state is None:
            ...
            self.state = struct.unpack("%iI" % (md5.digest_size >> 2), bytes.fromhex(state))
...
def padding(msg_bits):
    ...
    fmat = "".join(map(chr, _encode((msg_bits & 0xffffffff, msg_bits>>32), 8)))
    newPad = PADDING[:padLen] + fmat
    return newPad
...
#'\xf7\xbdakl\x84\x1d%8s_v\xd1\xe0+W'
def _encode(input, len):
    k = len >> 2
    args = ("%iI" % k)
    s = struct.Struct(args)
    res = s.pack(*input[:k])
    return res
    #fmat = "".join(map(chr, res))
    #print(repr(fmat))
#return fmat
...
def _decode(input, len):
    ...
    for i in range(0,len, 4):
        res.append(ord(input[i+3]) << 24 | ord(input[i+2]) << 16 | ord(input[i+1]) << 8 | ord(input[i]))
    return res
